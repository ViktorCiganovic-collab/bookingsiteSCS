export const translationKeys = {
  // 🗂️ Kategori: Microsoft Office Specialist
  "Microsoft Office Specialist": "categories.microsoft_office_specialist.name",
  "Certifieringstest ger bevis på kompetens som både arbetsgivare och medarbetare har nytta av. För företag är kunskapstest och certifiering ett bra sätt att mäta utbildningsbehov och resultat. Med certifierad personal minskar supportkostnader eftersom produktivitet och tekniska kunskaper ökar, dessutom är certifierad personal en kvalitetsgaranti för kunder. För medarbetaren innebär ett certifikat och en certifiering ett erkännande av specialistkunskaper.": "categories.microsoft_office_specialist.description",

  // 📜 Certifieringar
  "Word 365/2019 Associate": "categories.microsoft_office_specialist.certs.word_associate.name",
  "Grundläggande kunskaper i att skapa, formatera och redigera professionella dokument i Word.": "categories.microsoft_office_specialist.certs.word_associate.description",

  "Excel 365/2019 Associate": "categories.microsoft_office_specialist.certs.excel_associate.name",
  "Grundläggande färdigheter i kalkylblad, formler och datahantering i Excel.": "categories.microsoft_office_specialist.certs.excel_associate.description",

  "Outlook 365/2019 Associate": "categories.microsoft_office_specialist.certs.outlook_associate.name",
  "Effektiv hantering av e-post, kalender och kontakter i Outlook.": "categories.microsoft_office_specialist.certs.outlook_associate.description",

  "PowerPoint 365/2019 Associate": "categories.microsoft_office_specialist.certs.powerpoint_associate.name",
  "Skapa engagerande och visuellt tilltalande presentationer med PowerPoint.": "categories.microsoft_office_specialist.certs.powerpoint_associate.description",

  "Word 365/2019 Expert": "categories.microsoft_office_specialist.certs.word_expert.name",
  "Avancerad dokumenthantering, automatisering och samarbetsfunktioner i Word.": "categories.microsoft_office_specialist.certs.word_expert.description",

  "Excel 365/2019 Expert": "categories.microsoft_office_specialist.certs.excel_expert.name",
  "Expertkunskaper i avancerade funktioner, pivottabeller och dataanalys i Excel.": "categories.microsoft_office_specialist.certs.excel_expert.description",

  "Access 365/2019 Expert": "categories.microsoft_office_specialist.certs.access_expert.name",
  "Skapa och hantera komplexa databaser och rapporter med Access.": "categories.microsoft_office_specialist.certs.access_expert.description",


  // Microsoft Fundamentals Certifieringar
  "Microsoft Fundamentals Certifieringar": "categories.microsoft_fundamentals.name",
  "Här erbjuder vi ett utbud av grundläggande certifieringar över olika Microsoft-teknologier, vilket ger en solid grund för din karriär inom IT. Dessa certifieringar är utformade för både tekniska och icke-tekniska yrkesverksamma som vill förbättra sin förståelse för Microsofts teknologier.": "categories.microsoft_fundamentals.description",

  "Azure Fundamentals (AZ-900)": "categories.microsoft_fundamentals.certs.azure_fundamentals.name",
  "Introduktion till molntjänster och Microsoft Azure för nybörjare, även för icke-tekniska.": "categories.microsoft_fundamentals.certs.azure_fundamentals.description",

  "Azure AI Fundamentals (AI-900)": "categories.microsoft_fundamentals.certs.azure_ai_fundamentals.name",
  "Grundläggande AI och maskininlärning på Azure för nyfikna på AI-teknik.": "categories.microsoft_fundamentals.certs.azure_ai_fundamentals.description",

  "Azure Data Fundamentals (DP-900)": "categories.microsoft_fundamentals.certs.azure_data_fundamentals.name",
  "Basfakta om datahantering, analys och stordata för datadrivna beslut.": "categories.microsoft_fundamentals.certs.azure_data_fundamentals.description",

  "Security, Compliance, and Identity Fundamentals (SC-900)": "categories.microsoft_fundamentals.certs.security_compliance_identity.name",
  "Få grundläggande insikter i cybersäkerhet, efterlevnad och identitetshantering – perfekt för dig som vill förstå säkerhetsgrunderna i en digital värld.": "categories.microsoft_fundamentals.certs.security_compliance_identity.description",

  "Dynamics 365 Fundamentals (CRM)": "categories.microsoft_fundamentals.certs.dynamics_crm.name",
  "Utforska kundengagemangsverktyg i Dynamics 365 och förstå hur du skapar starkare kundrelationer genom smart teknik.": "categories.microsoft_fundamentals.certs.dynamics_crm.description",

  "Dynamics 365 Fundamentals (ERP)": "categories.microsoft_fundamentals.certs.dynamics_erp.name",
  "Lär dig grunderna i ERP-lösningar inom Dynamics 365 – och hur du effektiviserar ekonomi, drift och affärsprocesser med rätt verktyg.": "categories.microsoft_fundamentals.certs.dynamics_erp.description",

  "Power Platform Fundamentals (PL-900)": "categories.microsoft_fundamentals.certs.power_platform.name",
  "Automatisera, analysera och bygg lösningar utan kod. Power Platform gör det möjligt för alla att lösa affärsproblem smartare.": "categories.microsoft_fundamentals.certs.power_platform.description",

  "Microsoft 365 Fundamentals (MS-900)": "categories.microsoft_fundamentals.certs.microsoft_365.name",
  "Upptäck hur Microsoft 365:s appar förenklar samarbete och ökar produktivitet – oavsett var du arbetar ifrån.": "categories.microsoft_fundamentals.certs.microsoft_365.description",

    // Unity Certified User
  "Unity Certified User": "categories.unity_certified_user.name",
  "Unity är den ledande innehållsskapande plattformen för VR/AR, 3D i realtid och mobil 2D. Unity används av studenter och yrkesverksamma runt om i världen för att skapa allt från spel till filmer, 3D-modeller till VR-simuleringar.\n\nUnity Certified User certifiering är ingångsnivån på certifieringarna som gör det möjligt för individer att komma igång i interaktivt innehåll skapande för branscher som spel, underhållning, bil, AEC och XR.\n\nUnity Certified User-certifieringen hjälper arbetsgivare och institutioner att verifiera en kandidats kunskaper och färdigheter med Unity-teknik för att lyckas i dessa branscher. Oavsett om det är en önskan att skapa spel och appar eller bygga nya världar i AR/ VR, är Unity Certified User certifiering platsen att börja": "categories.unity_certified_user.description",

    // Adobe Certified Professional
  "Adobe Certified Professional": "categories.adobe_certified_professional.name",
  "Om du tar något av InDesign, Photoshop och Illustrator tillsammans med en annan Adobe certifiering kvalificerar du för Adobe Certified Professional.": "categories.adobe_certified_professional.description",

  "Adobe After Effects": "categories.adobe_certified_professional.certs.after_effects.name",
  "Skapa imponerande rörlig grafik och visuella effekter för video, webb och film – perfekt för dig som vill ta dina animationer till nästa nivå.": "categories.adobe_certified_professional.certs.after_effects.description",

  "Adobe Animate": "categories.adobe_certified_professional.certs.animate.name",
  "Bygg interaktiva animationer för webb, appar och spel. Animate är verktyget för dig som vill skapa livfullt, koddrivet innehåll i HTML5 och mer.": "categories.adobe_certified_professional.certs.animate.description",

  "Adobe Dreamweaver": "categories.adobe_certified_professional.certs.dreamweaver.name",
  "Designa och koda moderna, responsiva webbplatser. Dreamweaver kombinerar visuell design med kraften i kod – för både nybörjare och proffs.": "categories.adobe_certified_professional.certs.dreamweaver.description",

  "Adobe Illustrator": "categories.adobe_certified_professional.certs.illustrator.name",
  "Skapa vektorbaserad grafik som håller kvaliteten i alla storlekar. Illustrator är standardverktyget för logotyper, ikoner och illustrationer.": "categories.adobe_certified_professional.certs.illustrator.description",

  "Adobe InDesign": "categories.adobe_certified_professional.certs.indesign.name",
  "Bygg proffsiga trycksaker och digitala publikationer. InDesign är förstahandsvalet för designers som arbetar med layout för magasin, broschyrer och e-böcker.": "categories.adobe_certified_professional.certs.indesign.description",

  "Adobe Photoshop": "categories.adobe_certified_professional.certs.photoshop.name",
  "Redigera bilder, skapa digital konst eller designa grafik med det mest kraftfulla verktyget inom bildbehandling. Photoshop sätter standarden.": "categories.adobe_certified_professional.certs.photoshop.description",

  "Adobe Premiere Pro": "categories.adobe_certified_professional.certs.premiere_pro.name",
  "Kraftfull videoredigering för allt från sociala medier till långfilm. Premiere Pro ger dig total kontroll över din berättelse i rörligt format.": "categories.adobe_certified_professional.certs.premiere_pro.description",

    // Cisco Certified Support Technician
  "Cisco Certified Support Technican": "categories.cisco_certified_support_technician.name",
  "Detta program innehåller två certifieringsprov som är utformade för att validera arbetsklara färdigheter och kunskaper för att hjälpa eleverna att hitta ett jobb inom nätverks- och cybersäkerhetsområdet. Att få en Cisco Certified Support Technician-certifiering visar elevens färdigheter för arbetsgivare och berättar för dem att de är redo att kavla upp ärmarna från dag ett.\n\nCisco Certified Support Technician-programmet består av två prov, Cisco Certified Support Technician Networking och Cisco Certified Support Technician Cybersecurity.\n\nKontakta oss om du vill ha hjälp med kurser och övningstest.": "categories.cisco_certified_support_technician.description",

  "CCST Networking": "categories.cisco_certified_support_technician.certs.ccst_networking.name",
  "Testar dina nätverkskunskaper och förbereder dig för roller som nätverksanalytiker, supporttekniker och mer. Med CCST Networking-certifieringen visar du arbetsgivare att du är redo att ta itu med utmaningar från dag ett.": "categories.cisco_certified_support_technician.certs.networking.description",

  "CCST Cybersecurity": "categories.cisco_certified_support_technician.certs.ccst_cybersecurity.name",
  "Förbered dig för en karriär inom cybersäkerhet med detta certifikat. Som CCST Cybersecurity-certifierad har du de nödvändiga färdigheterna för att hantera säkerhetsincidenter och skydda organisationers digitala tillgångar.": "categories.cisco_certified_support_technician.certs.cybersecurity.description",

    // IT Specialist   
  "IT Specialist": "categories.it_specialist.name",
  "Dessa certifieringar är ett sätt för deltagare att validera grundläggande IT-kompetenser eftertraktade av arbetsgivare. IT Specialist-programmet är riktat mot individer som överväger eller precis börjat en karriär inom IT.": "categories.it_specialist.description",

  "Artificial Intelligence": "categories.it_specialist.certs.ai.name",
  "Deltagare för denna certifiering är AI-aktiverade elever som är förberedda för professionellt bruk av AI genom att förstå hur AI kan användas för att lösa problem.": "categories.it_specialist.certs.ai.description",

  "Cloud Computing": "categories.it_specialist.certs.cloud_computing.name",
  "Deltagare för denna certifiering kommer att kunna identifiera och beskriva slutlösningar som utnyttjar molnteknologier.": "categories.it_specialist.certs.cloud_computing.description",

  "Computational Thinking": "categories.it_specialist.certs.computational_thinking.name",
  "Deltagare för denna certifiering dekomponerar problem, samlar in och analyserar data, känner igen mönster i data, representerar data genom abstraktioner och automatiserar lösningar genom algoritmiskt tänkande.": "categories.it_specialist.certs.computational_thinking.description",

  "Cybersecurity": "categories.it_specialist.certs.cybersecurity.name",
  "Deltagare för denna certifiering inleder sin resa inom cybersäkerhetsområdet. Denna certifiering bedömer deras förståelse för nyckelsäkerhetsparadigm, terminologi och tankesätt.": "categories.it_specialist.certs.cybersecurity.description",

  "Data Analytics": "categories.it_specialist.certs.data_analysis.name",
  "Deltagare för denna certifiering söker att bevisa grundläggande kunskap om hur man ansvarsfullt manipulerar, analyserar och kommunicerar resultat av dataanalys.": "categories.it_specialist.certs.data_analysis.description",

  "Databases": "categories.it_specialist.certs.databases.name",
  "Deltagare för denna certifiering kommer att visa grundläggande kunskap om hur man designar och frågar relationsdatabaser.": "categories.it_specialist.certs.databases.description",

  "Device Configuration and Management": "categories.it_specialist.certs.device_configuration.name",
  "Deltagare för denna certifiering kommer att visa grundläggande färdigheter och kunskap om Windows-enheter och mobilitet.": "categories.it_specialist.certs.device_configuration.description",

  "HTML and CSS": "categories.it_specialist.certs.html_css.name",
  "Deltagare för denna certifiering kommer att visa att de kan strukturera en webbsida med HTML-element och skapa och tillämpa stilar med CSS.": "categories.it_specialist.certs.html_css.description",

  "HTML5 Application Development": "categories.it_specialist.certs.html5_app.name",
  "Deltagare för denna certifiering kommer att demonstrera sin förmåga att använda HTML5, CSS och JavaScript för att bygga responsiva webbapplikationer som fungerar på olika pekutrustade enheter, inklusive datorer, surfplattor och telefoner.": "categories.it_specialist.certs.html5_app.description",

  "Java": "categories.it_specialist.certs.java.name",
  "Deltagare för denna certifiering kommer att visa att de kan känna igen, skriva och felsöka Java-kod som logiskt löser ett problem.": "categories.it_specialist.certs.java.description",

  "JavaScript": "categories.it_specialist.certs.javascript.name",
  "Deltagare för denna certifiering kommer att visa att de kan känna igen, skriva och felsöka JavaScript-kod som logiskt löser ett problem.": "categories.it_specialist.certs.javascript.description",

  "Networking": "categories.it_specialist.certs.networking.name",
  "Deltagare för denna certifiering kommer att visa grundläggande nätverkskunskaper och färdigheter, inklusive TCP/IP, nätverkstjänster, nätverkstopologier och felsökning i trådbundna och trådlösa miljöer.": "categories.it_specialist.certs.networking.description",

  "Network Security": "categories.it_specialist.certs.network_security.name",
  "Deltagare för denna certifiering kommer att visa grundläggande säkerhetskunskaper och färdigheter, inklusive förståelse för core säkerhetsprinciper, operativsystemets säkerhet, nätverks- och enhetssäkerhet samt säkra datorpraxis.": "categories.it_specialist.certs.network_security.description",

  "Python": "categories.it_specialist.certs.python.name",
  "Deltagare för denna certifiering kommer att visa att de kan känna igen, skriva och felsöka Python-kod som logiskt löser ett problem.": "categories.it_specialist.certs.python.description",

  "Software Development": "categories.it_specialist.certs.software_development.name",
  "Deltagare för denna certifiering kommer att visa kärnkompetenser inom programvaruutveckling, inklusive objektorienterad programmering, webbapplikationer och databaser. Deltagare förväntas ha viss erfarenhet av C# och ANSI SQL.": "categories.it_specialist.certs.software_development.description",
  

  //Unity certified User certifications
  "Unity Certified User Programmer": "categories.unity_certified_user_programmer.name",
  "Certifieringsprovet Unity Certified User Programmer validerar grundläggande färdigheter i C#‑programmering inom Unity‑miljön. Kandidater visar sin förmåga att skapa interaktivitet i spel, applikationer, AR/VR och andra upplevelser.  Denna grundnivåcertifiering omfattar centrala kompetenser som felsökning, problemlösning, tolkning av API:er, att skriva och utvärdera kod samt att navigera i Unitys gränssnitt. För att vara väl förberedd förväntas kandidater ha genomfört minst 150 timmars Unity‑träning innan provet.": "categories.unity_certified_user_programmer.description",

  "Detta certifieringsprov bedömer de grundläggande färdigheter som krävs för att bygga VR‑simulationer i Unity. Certifieringen bekräftar kandidatens förmåga att designa och utveckla interaktiva VR‑upplevelser och applikationer inom Unity‑miljön.": "categories.unity_certified_user_vr.description",
  "Denna certifiering bekräftar grundläggande färdigheter i digitalt skapande och visuell design inom Unity. Provet testar kandidatens förmåga att arbeta med 2D‑ och 3D‑assets, använda material och texturer, ljussättning och effekter, samt att skapa användargränssnitt och visuella kompositioner.  Certifieringen visar att kandidaten kan producera interaktiva och visuellt tilltalande upplevelser i spel, applikationer och AR/VR‑miljöer. För att vara väl förberedd rekommenderas minst 150 timmars träning i Unity.": "categories.unity_certified_user_design.description"


};
